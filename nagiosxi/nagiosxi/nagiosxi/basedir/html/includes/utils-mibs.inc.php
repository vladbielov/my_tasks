...
function mibs_get_associated_traps($mibs) {
    ...
    foreach ($mibs as $mib_name) {
        $sql = 'SELECT trapdata_event_name as event_name, trapdata_event_oid as oid, trapdata_category as category, trapdata_severity as severity from ' . $db_tables[DB_NAGIOSXI]["cmp_trapdata"] . " WHERE trapdata_parent_mib_name = '$mib_name'";
