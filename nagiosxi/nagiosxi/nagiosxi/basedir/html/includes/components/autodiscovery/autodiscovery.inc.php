...
function autodiscovery_component_get_cmdline($jobid)
{
    ...
    if (!empty($custom_ports)) {
        $custom_ports = "--customports=\"".escapeshellcmd($custom_ports)."\"";
    }
    
    $cmd = "rm -f " . $xml_file . "; touch " . $watch_file . "; sudo /usr/bin/php " . $script_dir . "autodiscover_new.php --addresses=\"" . escapeshellcmd($address) . "\"  --exclude=\"" . escapeshellcmd($exclude_address) . "\" --output=" . $xml_file . " --watch=" . $watch_file . " --onlynew=0 --debug=1 " . $osd . " " . $topod . " " . $scan_delay . " " . $system_dns . " " . $custom_ports . " > " . $out_file . " 2>&1 & echo $!";
