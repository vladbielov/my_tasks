    ...
    if (!empty($hosts)) {
        ...
        foreach ($hosts as $host) {
            ...
            if ($command == 'blank') {
                ...
                // Update old command with new options... but only the ones we need to
                ...
                if ($command == '') {
                    $command = $cmdarr[0];
                    $log = sprintf("Updated command arguments for host %s", nagiosql_get_host_name($host));
                } else {
                    $log = sprintf("Updated command for host %s to %s", nagiosql_get_host_name($host), nagiosql_get_object_name('command', $command));
                }
    ...
    if (!empty($services)) {
        ...
        foreach ($services as $service) {
            ...
            if ($command == 'blank') {
                ...
                if ($command == '') {
                    $command = $cmdarr[0];
                    $log = sprintf("Updated command arguments for service %s", nagiosql_get_service_name($service));
                } else {
                    $log = sprintf("Updated command for service %s to %s", nagiosql_get_service_name($service), nagiosql_get_object_name('command', $command));
                }
