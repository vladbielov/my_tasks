...
function get_ajax_relationship_table($opt = 'host')
{
    ...
    $query = "SELECT `id`,`host_name` FROM `tbl_lnkHostToContact` LEFT JOIN `tbl_host` ON `idMaster` = `id` WHERE `idSlave` = '{$id}' ORDER BY host_name ASC";
    ...
    if (get_user_meta(0, 'ccm_access') == 2 && !is_authorized_for_all_objects() && !is_admin()) {
        $hosts = array();
        $sql = "SELECT object_id FROM tbl_permission WHERE type = ".OBJECTTYPE_HOST." AND user_id = ".$_SESSION['user_id'];
    ...
    // Get option list
    $query = "SELECT `id`,`config_name`,`service_description` FROM `tbl_lnkServiceToContact` LEFT JOIN `tbl_service` ON `idMaster` = `id` WHERE `idSlave` = '{$id}' ORDER BY config_name, service_description ASC";
    $results = exec_sql_query(DB_NAGIOSQL, $query, true);

    if (get_user_meta(0, 'ccm_access') == 2 && !is_authorized_for_all_objects() && !is_admin()) {
        $services = array();
        $sql = "SELECT object_id FROM tbl_permission WHERE type = ".OBJECTTYPE_SERVICE." AND user_id = ".$_SESSION['user_id'];
        $rs = exec_sql_query(DB_NAGIOSQL, $sql, true);
        $objs = $rs->GetArray();
        foreach ($objs as $o) {
...
function get_cg_ajax_relationship_table($opt = 'host')
{
    ...
    $query = "SELECT `id`,`host_name` FROM `tbl_lnkHostToContactgroup` LEFT JOIN `tbl_host` ON `idMaster` = `id` WHERE `idSlave` = '{$id}' ORDER BY host_name ASC";
    $results = exec_sql_query(DB_NAGIOSQL, $query, true);
    ...
    // Get option list
    $query = "SELECT `id`,`config_name`,`service_description` FROM `tbl_lnkServiceToContactgroup` LEFT JOIN `tbl_service` ON `idMaster` = `id` WHERE `idSlave` = '{$id}' ORDER BY config_name, service_description ASC";
    $results = exec_sql_query(DB_NAGIOSQL, $query, true);
...
function get_hg_ajax_relationship_table()
{
    ...
    $query = "SELECT `id`,`host_name` FROM `tbl_lnkHostToHostgroup` LEFT JOIN `tbl_host` ON `idMaster` = `id` WHERE `idSlave` = '{$id}' ORDER BY host_name ASC";
    $results = exec_sql_query(DB_NAGIOSQL, $query, true);
...
function get_sg_ajax_relationship_table()
{
    ...
    $query = "SELECT `id`,`config_name`, `service_description` FROM `tbl_lnkServiceToServicegroup` LEFT JOIN `tbl_service` ON `idMaster` = `id` WHERE `idSlave` = '{$id}'";
    $results = exec_sql_query(DB_NAGIOSQL, $query, true);
...
function get_ph_ajax_relationship_table()
{
    ...
    $query = "SELECT `id`,`host_name` FROM `tbl_lnkHostToHost` LEFT JOIN `tbl_host` ON `idMaster` = `id` WHERE `idSlave` = '{$id}' ORDER BY host_name ASC";
    $results = exec_sql_query(DB_NAGIOSQL, $query, true);
...
function get_free_variables_ajax_relationship_table()
{
    ...
    if (get_user_meta(0, 'ccm_access') == 2 && !is_authorized_for_all_objects() && !is_admin()) {
        $contacts = array();
        $sql = "SELECT object_id FROM tbl_permission WHERE type = " . OBJECTTYPE_CONTACT. " AND user_id = ". $_SESSION['user_id'];
        $rs = exec_sql_query(DB_NAGIOSQL, $sql, true);
        $objs = $rs->GetArray();
        foreach ($objs as $o) {
