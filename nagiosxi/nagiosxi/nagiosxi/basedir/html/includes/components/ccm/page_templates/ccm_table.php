...
function ccm_table($args, $returnContent=array(0, ''), $page=1)
{
    ...
    {   
        ...
        if (!$ccm_restricted) {
            $icons .= "<span class='action ccm-tt-bind' title='"._('View config')."' onclick=\"actionPic('download', '".encode_form_valq($cfg_name)."', '{$_SESSION['domain']}')\"><img src='images/detail.png' alt='Download'></span>";
        }
...
function process_desc_exceptions(&$desc, $type, $data, $args, $id)
{
    ...
    if ($type == 'serviceescalation' || $type == 'servicedependency') {
        ...
        if ($data[$args['keyDesc']] == 1) {
            ...
            $query = "SELECT `id`, `service_description` FROM `tbl_service` LEFT JOIN `{$table}` ON `id`=`idSlave` WHERE `idMaster`=".$id;
